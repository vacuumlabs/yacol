...
Parent coroutine is responsible for handling errors thrown (and not handled) by all its children
(i.e. other coroutines it created) during their whole lifespan.
...
If the error is not caught, it is propagated to parent, grandparent, etc. until a coroutine with
a registered `.catch` handler is found. This handler is then executed; if it ends normally with value
`x`, the error is considered to be handled. The coroutine that handled the error is stopped and is
considered successfully terminated with the return value `x`. All other coroutines are stopped and
considered failed. This simple example demonstrates this behavior:
...
```
After running this example, `cor4` and `cor3` are failed (`cor3` caught the error, but rethrowed it), `cor2`
ended successfully with result 42. Error handler of `cor1` is never executed.

# Coroutine Termination

Imagine you see the following piece of code:
```javascript
await doSomeJob()
```
...
even after `doSomeJob()` has 'officially' ended.

In Yacol, coroutine terminates only after its generator returns and all its child coroutines terminate.
...
it has to wait until its child `cor2` terminates, so it also terminates after 2 seconds.

# Messaging
Yacol implements simple CSP-inspired messaging system. Adding messages to channel happens instantly,
yielding messages can block (if the queue is empty). The example below illustrates a simple
...
Semantically, killing is equivalent to all coroutines (and its children) getting to error
...
`context.get(key, val)` will look for the value first in the current coroutine context, and if not
found, it will try to read from parent's context, grandparent's context, etc. Check out
`examples/context.js`.
...
define the meaning of 'FETCH_WEATHER' for you. Note that this has exactly the power of continuations of
OCaml - we can write continuation-like code without any babel transpiling! Check out
`examples/patch.js` to see what weather is in my city right now!

# Inspect
If you call `.inspect()` on a coroutine, you can inspect what effects the coroutine tries to
...
```
This allows us to assert what effects the coroutine tries to execute without writing a
single mock!
...
It has correctly implemented `.then` and `.catch` methods. For example, this means that a coroutine can be
returned from a Mocha test, Mocha will wait until the coroutine (and all spawned sub-coroutines) are
terminated. If any of these produce (unhandled) error, this goes to top-level `.catch` handler and
the test is therefore rejected.

### Working with Express
A coroutine can be used as an express request handler and / or an express middleware. With a little
...
Furthermore, this works nicely with other express middlewares / handlers which are used in the standard way, so you can combine `register` from above with the standard approach:
...
# Stacktraces
Let me first illustrate the problem. Having the code like this (`examples/bad-stacktrace.js`):
```javascript
async function a() {
...
Although, there is `file.js` mentioned multiple times, you won't find anything useful at line 6 or
line 9. Line 60 does not even exist in this file! The only useful information in this long
...
Let us see what stacktrace a similar code written in yacol gives us (`examples/awesome-stacktrace.js`)
...
Not only you can see all the relevant coroutines calling each other, you can even see the names of functions
